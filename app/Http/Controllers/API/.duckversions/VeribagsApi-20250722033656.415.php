...
class VeribagsApi   extends Controller
{
    ...
    public function processBarcodeData(Request $request)
    {
        ...
        // Remove only leading zeros from flight number
        // Examples: '0685' -> '685', '0850' -> '850', '9650' -> '9650', '0800' -> '800'
        $trimmedFlightNumber = ltrim($flightNumber, '0');
        // If all digits were zeros, keep at least one zero
        if ($trimmedFlightNumber === '') {
            $trimmedFlightNumber = '0';
        }
        ...
        $departureData = DepartureMovementView::
        where('DepartureMovement_View.FlightId',$flightId)
        ...
        // dd($departureData, $flightId, $startDate, $endDate, $formattedDate);
//         dd($departureData, $flightId, $startDate, $endDate, $formattedDate, 
//    DepartureMovementView::where('DepartureMovement_View.FlightId', '=', $flightId)
//     ->where('DepartureMovement_View.ScheduledDatetime', '>=', $startDate)
//     ->where('DepartureMovement_View.ScheduledDatetime', '<=', $endDate)
//     ->toSql());

$testQuery = DepartureMovementView::where('DepartureMovement_View.FlightId', $flightId)->first();
dd("Simple query result:", $testQuery);

// Hoặc thử truy vấn với LIKE thay vì =
$testQuery2 = DepartureMovementView::where('FlightId', 'like', '%' . $flightId . '%')->first();
dd("LIKE query result:", $testQuery2);
